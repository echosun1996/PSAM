...
def parse_args():
    ...
    parser.add_argument(
        "-mod", type=str, default="sam_adpt", help="mod type:seg,cls,val_ad"
    )
    parser.add_argument("-exp_name", default="msa_test_isic", type=str, help="net type")
    parser.add_argument(
        "-type", type=str, default="map", help="condition type:ave,rand,rand_map"
    )
    ...
    parser.add_argument(
        "-val_freq", type=int, default=5, help="interval between each validation"
    )
    ...
    parser.add_argument(
        "-w", type=int, default=4, help="number of workers for dataloader"
    )
    ...
    parser.add_argument(
        "-imp_lr", type=float, default=3e-4, help="implicit learning rate"
    )
    parser.add_argument(
        "-weights", type=str, default=0, help="the weights file you want to test"
    )
    parser.add_argument(
        "-base_weights", type=str, default=0, help="the weights baseline"
    )
    parser.add_argument("-sim_weights", type=str, default=0, help="the weights sim")
    parser.add_argument(
        "-distributed", default="none", type=str, help="multi GPU ids to use"
    )
    ...
    parser.add_argument(
        "-data_path", type=str, default="../data", help="The path of segmentation data"
    )
    # '../dataset/RIGA/DiscRegion'
    # '../dataset/ISIC'
    parser.add_argument("-output_path", type=str, default=None, help="output path")
    opt = parser.parse_args()
    if opt.output_path is not None and not os.path.exists(opt.output_path):
