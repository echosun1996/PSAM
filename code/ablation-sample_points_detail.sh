...
# 修改该变量用于控制是否使用详细的正样本点
positive_detail=False
n_sample_points_detail=1

for n_sample_points_detail in $(seq 1 8)
do

if [ "$n_sample_points_detail" -eq 4 ]; then
    continue
fi

echo "Processing with n_sample_points_detail = $n_sample_points"
...
if [ "$positive_detail" = True ]; then
    output_folder_name="PSAM_cp_positive_"$n_sample_points_detail"_"$prompt_type
else
    output_folder_name="PSAM_cp_negative_"$n_sample_points_detail"_"$prompt_type
fi
...
if [ "$positive_detail" = True ]; then
    wandb_comment="Use "$n_sample_points_detail" positive points and 4 negative points; "$wandb_comment
else
    wandb_comment="Use "$n_sample_points_detail" negative points and 4 positive points; "$wandb_comment
fi
...
#     cd $parent_dir/code
#     if [ "$scribble_type" = "" ]; then
#         CUDA_VISIBLE_DEVICES=$gpu_id torchrun  --master_port=$(( RANDOM % 49152 + 10000 )) --nproc_per_node=$gpu_sum train.py --checkpoint ./pretrained_checkpoint/sam_vit_b_01ec64.pth --model-type vit_b --output $parent_dir/results/$output_folder_name --input $parent_dir/data --input_size $img_resize $img_resize --max_epoch_num $max_epoch_num --model_save_fre 2 --logfile $parent_dir/results/$output_folder_name/PSAM_train.txt  --comments "$wandb_comment" --labeller $labeller --complementary-points $n_sample_points --scribble-positive-type "lineScribble"  --scribble-negative-type "lineScribble"
...
if [ "$evaluate_model" = true ]; then
    ...
    # cp $parent_dir/results/PSAM/epoch_16.pth $parent_dir/results/$output_folder_name/epoch_16.pth
    ...
    if [ "$scribble_type" = "" ]; then
        if [ "$positive_detail" = True ]; then
            CUDA_VISIBLE_DEVICES=$gpu_id torchrun --nnodes=1 --nproc_per_node=1  --master_port=$(( $RANDOM % 49152 + 10000 )) train.py --eval --checkpoint ./pretrained_checkpoint/sam_vit_b_01ec64.pth --model-type vit_b --output $parent_dir/results/$output_folder_name --restore-model $parent_dir/results/PSAM/epoch_16.pth --input $parent_dir/data --input_size $img_resize $img_resize --logfile $parent_dir/results/$output_folder_name/PSAM_eva.txt --visualize  --comments "$wandb_comment" --labeller $labeller --n-negative-points 4 --n-positive-points $n_sample_points_detail
        else
            CUDA_VISIBLE_DEVICES=$gpu_id torchrun --nnodes=1 --nproc_per_node=1  --master_port=$(( $RANDOM % 49152 + 10000 )) train.py --eval --checkpoint ./pretrained_checkpoint/sam_vit_b_01ec64.pth --model-type vit_b --output $parent_dir/results/$output_folder_name --restore-model $parent_dir/results/PSAM/epoch_16.pth --input $parent_dir/data --input_size $img_resize $img_resize --logfile $parent_dir/results/$output_folder_name/PSAM_eva.txt --visualize  --comments "$wandb_comment" --labeller $labeller --n-positive-points 4 --n-negative-points $n_sample_points_detail
        fi
