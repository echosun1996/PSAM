...
def show_anns_ax(
    ax, masks, input_point, input_box, input_label, filename, image, ious, boundary_ious
):
    if len(masks) == 0:
        return
...
def mask_opt(masks):
    ...
    for mask in masks:
        ...
        for contour in contours:
            ...
            if area > max_area:
                max_area = area
                temp_mask = np.zeros(mask_np.shape, dtype=np.uint8)
                cv2.drawContours(temp_mask, contour, -1, (255), thickness=cv2.FILLED)
                cv2.fillPoly(temp_mask, [contour], 255)
                temp_max_area_mask = temp_mask.copy()
        if max_area_mask == None:
            max_area_mask = temp_max_area_mask
        else:
            max_area_mask.append(temp_max_area_mask)
    max_area_mask = torch.tensor(max_area_mask, dtype=torch.bool)
...
def show_anns(
    masks, input_point, input_box, input_label, filename, image, ious, boundary_ious
):
    if len(masks) == 0:
        return
