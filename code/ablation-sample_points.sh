...
for n_sample_points in $(seq 1 10)
do
echo "Processing with n_sample_points = $n_sample_points"
...
output_folder_name="PSAM_cp_"$n_sample_points"_"$prompt_type
...
if [ "$evaluate_model" = true ]; then
    ...
    if [ "$scribble_type" = "" ]; then
        CUDA_VISIBLE_DEVICES=$gpu_id torchrun --nnodes=1 --nproc_per_node=1  --master_port=$(( RANDOM % 49152 + 10000 )) train.py --eval --checkpoint ./pretrained_checkpoint/sam_vit_b_01ec64.pth --model-type vit_b --output $parent_dir/results/$output_folder_name --restore-model $parent_dir/results/PSAM/epoch_16.pth --input $parent_dir/data --input_size $img_resize $img_resize --logfile $parent_dir/results/$output_folder_name/PSAM_eva.txt --visualize  --comments "$wandb_comment" --labeller $labeller --n-sample-points $n_sample_points
    # elif [ "$scribble_type" = "LinePAndLineN" ]; then
