...
class WarpScribble:
    ...
    def noise_mask(
        self, shape: Union[Tuple[int], List[int]] = (8, 128, 128), device=None
    ):
        """
    ...
    def apply_warp(self, x: torch.Tensor):
        ...
        if x.sum() > 0:
            ...
            warped = voxynth.transform.spatial_transform(
                x, trf=deformation_field, isdisp=False
            )
            if warped.sum() == 0:
                return x
            else:
    ...
    def __call__(self, mask: torch.Tensor, n_scribbles: int = 1) -> torch.Tensor:
        ...
        assert len(mask.shape) in [
            3,
            4,
        ], f"mask must be b x 1 x h x w or 1 x h x w. currently {mask.shape}"
    ...
    def batch_scribble(self, mask: torch.Tensor, n_scribbles: int = 1) -> torch.Tensor:
        ...
        if self.warp:
            warped_scribbles = torch.stack(
                [self.apply_warp(scribbles[b, ...]) for b in range(bs)]
            )  # shape: b x 1 x H x W
        else:
        ...
        if self.max_pixels is not None:
            ...
            noise = torch.stack(
                [
                    voxynth.noise.perlin(
                        shape=mask.shape[-2:],
                        smoothing=self.max_pixels_smooth,
                        magnitude=1,
                        device=mask.device,
                    )
                    for _ in range(bs)
            ...
            corrected_warped_scribbles = (
                binary_mask.view(*mask.shape) * corrected_warped_scribbles
            )
        ...
        if self.show:
            ...
            if bs > 1:
                for i in range(bs):
                    show_scribbles(
                        corrected_warped_scribbles[i, 0, ...].cpu(), axes[i, -1]
                    )
            else:
...
class CenterlineScribble(WarpScribble):
    ...
    def batch_scribble(self, mask: torch.Tensor, n_scribbles: Optional[int] = 1):
        ...
        assert (
            len(mask.shape) == 4
        ), f"mask must be b x 1 x h x w. currently {mask.shape}"
        bs = mask.shape[0]
                ...
                np.stack(
                    [
                        cv2.ximgproc.thinning(
                            mask_w_border[i, ...].cpu().numpy().astype(np.uint8)
                        )
                        / 255
                        for i in range(bs)
                    ]
        ...
        if self.dilate_kernel_size is not None:
            ...
            if k > 0:
                ...
                dilated_skeleton = kornia.morphology.dilation(
                    skeleton, kernel=kernel, engine="convolution"
                )
        else:
        ...
        if self.show:
            ...
            if self.max_pixels is not None:
                tensors += [
                    ...
                    warped_scribbles,
                    noise,
                    binary_mask.reshape(*mask.shape),
                    corrected_warped_scribbles,
                    mask,
                ]
                titles += [
            ...
            fig, axes = ne.plot.slices(
                sum(
                    [[x[i, ...].squeeze().cpu() for x in tensors] for i in range(bs)],
                    [],
                ),
                sum([titles for _ in range(bs)], []),
                show=False,
...
class ContourScribble(WarpScribble):
    ...
    def __init__(
        ...
        self.blur_fn = kornia.augmentation.RandomGaussianBlur(
            kernel_size=(blur_kernel_size, blur_kernel_size), sigma=blur_sigma, p=1.0
        )
        # Line thickness
        self.dilate_kernel_size = dilate_kernel_size
        # Corrections
    ...
    def batch_scribble(self, mask: torch.Tensor, n_scribbles: Optional[int] = 1):
        ...
        corrected_blur_mask = torch.reshape(
            torch.maximum(blur_mask, rev_mask), (bs, -1)
        )
        ...
        thresh = torch.from_numpy(np.random.uniform(min_bs, max_bs, size=bs)).to(
            mask.device
        )
        ...
        if self.dilate_kernel_size is not None:
            ...
            if k > 0:
                ...
                dilated_boundary = kornia.morphology.dilation(
                    boundary, kernel=kernel, engine="convolution"
                )
            else:
        ...
        if self.preserve_scribble:
            # If none of the scribble falls in the mask after warping, remove the warping
            idx = torch.where(
                torch.sum(corrected_warped_scribbles, dim=(1, 2, 3)) == 0
            )[0]
            corrected_warped_scribbles[idx, ...] = mask[idx, ...] * scribbles[idx, ...]
        ...
        if self.show:
            ...
            tensors = [
                mask,
                blur_mask.view(mask.shape),
                corrected_blur_mask.view(mask.shape),
                binary_blur_mask,
                boundary,
            ]
            titles = [
                "Input Mask",
                "Blurred (Rev.) Mask",
                "Corrected Blurred Mask",
                "Thresholded Blur. Mask",
                "Contours",
            ]
...
class XScribble(WarpScribble):
    def __init__(
        self, warp: bool = False, warp_smoothing: int = 4, warp_magnitude: int = 16
    ):
        super().__init__(
            warp=warp, warp_smoothing=warp_smoothing, warp_magnitude=warp_magnitude
        )

    def generate_line_mask(
        self, shape: Tuple[int], angle: float, thickness: int = 1, mask=None
    ):
        """
        Generate a binary mask with a single line at a specified angle, truncated by the mask region.

        Args:
            shape: Tuple of the form (H, W) defining the mask size.
            angle: Angle in degrees for the line.
            thickness: Line thickness in pixels.
            mask: Optional mask tensor to limit the line.

        Returns:
            A binary mask of shape (1, H, W) with a truncated line.
        """
        H, W = shape
        ...
        if mask is not None:
            ...
            # Draw lines for each mask component
            for label_idx in range(1, num_labels):  # Skip the background label (0)
                x, y, w, h = stats[label_idx, cv2.CC_STAT_LEFT : cv2.CC_STAT_HEIGHT + 1]

                # Compute line start and end points based on bounding box and angle
                center = (y + h // 2, x + w // 2)
                length = max(w, h)

                # Convert angle to radians and calculate line endpoints
                angle_rad = np.deg2rad(angle)
                x0 = int(center[1] - length * math.cos(angle_rad) / 2)
                y0 = int(center[0] - length * math.sin(angle_rad) / 2)
                x1 = int(center[1] + length * math.cos(angle_rad) / 2)
                y1 = int(center[0] + length * math.sin(angle_rad) / 2)
            ...
            # Ensure that line_mask only exists within the region of mask_np
            line_mask = cv2.bitwise_and(line_mask, mask_np)

        return torch.tensor(line_mask, dtype=torch.int32).unsqueeze(0)

    def batch_scribble(self, mask: torch.Tensor, n_scribbles: int = 1):
        """
        Simulate X-shaped scribbles for a batch of examples (mask), truncated by non-mask regions.
        """
        b, _, H, W = mask.shape
        scribble_mask = torch.zeros_like(mask)

        # Generate lines for each batch element
        for i in range(b):
            angle_a = np.random.uniform(22.5, 67.5)
            angle_b = np.random.uniform(112.5, 157.5)

            # Truncate the line within each mask region
            angle1_mask = self.generate_line_mask(
                (H, W), angle=angle_a, thickness=2, mask=mask[i]
            )
            angle2_mask = self.generate_line_mask(
                (H, W), angle=angle_b, thickness=2, mask=mask[i]
            )
            combined_mask = torch.maximum(angle1_mask, angle2_mask)
            scribble_mask[i] = combined_mask

        # Optionally apply warping
        if self.warp:
            scribble_mask = self.apply_warp(scribble_mask)
