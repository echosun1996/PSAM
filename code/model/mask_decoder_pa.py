...
class PromptAdapater(nn.Module):
    ...
    def __init__(
        ...
        self.uncertain_mlp = MLP(
            embedding_dim * 2, embedding_dim // 4, embedding_dim, 3
        )
    ...
    def forward(
        ...
        query_pe: Tensor,
        key_pe: Tensor,
        early_embeddings: Tensor,
        final_embeddings: Tensor,
        prompt_encoder,
        image_record,
        guiding_embedding,
        v_coarse,
        ...
        attn_out, v_refined = self.cross_attn_token_to_image.forward_return_v(
            q=queries + query_pe, k=guiding_embedding + key_pe, v=guiding_embedding
        )
        queries = attn_out

        # generating uncertain token and refined token
        uncertain_token = torch.cat(
            [self.static_uncertain_token.weight.repeat(b, 1), queries[:, 1, :]], dim=-1
        )
        uncertain_token = self.uncertain_mlp(uncertain_token).unsqueeze(1)
        refined_token = torch.cat(
            [self.static_refined_token.weight.repeat(b, 1), queries[:, 1, :]], dim=-1
        )
        ...
        final_mask = (unceratinty_map_norm >= 0.5).detach() * refined_mask + (
            unceratinty_map_norm < 0.5
        ).detach() * coarse_mask

        point_sample_ref_map = unceratinty_map_norm * (
            torch.sigmoid(refined_mask) - torch.sigmoid(coarse_mask)
        )

        mask = {
            "unceratinty_map": unceratinty_map_norm,
            "refined_mask": refined_mask,
            "coarse_mask": coarse_mask,
            "final_mask": final_mask,
            "ref_map": point_sample_ref_map,
        }

        # replace mask token with refined token
        queries = torch.cat(
            [queries[:, 0:1, :], output_refined_token, queries[:, 2:, :]], dim=1
        )
        ...
        if int(os.environ.get("CURRENT_EPOCH", 0)) > 4 or not self.training:
            ...
            gumbel_dist = torch.distributions.gumbel.Gumbel(
                torch.tensor(
                    ...
                    dtype=point_sample_ref_map.dtype,
                ),
                torch.tensor(
                    1.0,
                    device=point_sample_ref_map.device,
            ...
            if self.training:
                ret_p = self.gumbel_softmax_topk(
                    gumbel_dist, point_sample_ref_map, n_sample_points
                )  # B × HW
                ret_n = self.gumbel_softmax_topk(
                    gumbel_dist, -point_sample_ref_map, n_sample_points
                )
            else:
                if (
                    n_sample_points != n_positive_points
                    or n_positive_points != n_negative_points
                ):
                    topk_values, topk_indices = torch.topk(
                        point_sample_ref_map, k=n_positive_points, dim=1
                    )
                    khot = torch.zeros_like(
                        point_sample_ref_map,
                        memory_format=torch.legacy_contiguous_format,
                    )
                    ret_p = khot.scatter_(1, topk_indices, 1)
                    topk_values, topk_indices = torch.topk(
                        -point_sample_ref_map, k=n_negative_points, dim=1
                    )
                    khot = torch.zeros_like(
                        point_sample_ref_map,
                        memory_format=torch.legacy_contiguous_format,
                    ...
                    topk_values, topk_indices = torch.topk(
                        point_sample_ref_map, k=n_sample_points, dim=1
                    )
                    khot = torch.zeros_like(
                        point_sample_ref_map,
                        memory_format=torch.legacy_contiguous_format,
            ...
            y_embed = y_embed / 64
            x_embed = x_embed / 64
            pe = prompt_encoder.pe_layer._pe_encoding(
    ...
    def gumbel_softmax_topk(self, gumbel_dist, input, n_sample_points, tau=1):
...
class MaskDecoderPA(MaskDecoder):
    def __init__(self, model_type):
        ...
        if checkpoint_dict[model_type] and os.path.exists(checkpoint_dict[model_type]):
            checkpoint_path = checkpoint_dict[model_type]
            self.load_state_dict(torch.load(checkpoint_path))
        print("PA Decoder init from SAM MaskDecoder")
        for n, p in self.named_parameters():
            if (
                "output_upscaling" in n
                or "output_hypernetworks_mlps" in n
                or "mask_tokens" in n
            ):
                continue
        ...
        # -----------------------prompt adapter-------------------------------
        self.prompt_adapter = PromptAdapater(
            transformer_dim, self.transformer.num_heads
        )
    ...
    def forward(
        ...
        for i_batch in range(batch_len):
            ...
            image_grad = (
                misc.generalized_image_grad(input_images[i_batch]).unsqueeze(0) / 255
            )
            ...
            mask, iou_pred, interm_result = self.predict_masks(
                ...
                n_positive_points=n_positive_points,
            ...
            ref_maps.append(interm_result[0]["ref_map"])
            box_preds.append(interm_result[1])
            ret_ps.append(interm_result[2])
            ret_ns.append(interm_result[3])
        ...
        ref_maps = torch.cat(ref_maps, 0)
        # 去除 ret_ps ret_ns 中的 None 值
        ret_ps = [t for t in ret_ps if t is not None]
        ret_ns = [t for t in ret_ns if t is not None]
        if len(ret_ps) > 0:
            ret_ps = torch.cat(ret_ps, 0)
        else:
            # 处理 ret_ps 为空的情况，例如返回一个空 tensor 或执行其他逻辑
            ret_ps = torch.tensor([])

        if len(ret_ns) > 0:
            ret_ns = torch.cat(ret_ns, 0)
        else:
            # 处理 ret_ns 为空的情况，例如返回一个空 tensor 或执行其他逻辑
            ret_ns = torch.tensor([])
        # ret_ps = torch.cat(ret_ps, 0)
        # ret_ns = torch.cat(ret_ns, 0)
        box_preds = None
    ...
    def predict_masks(
        ...
        hs, src, Interm_result = self.transformer.forward_with_prompt_adapter(
            src, pos_src, tokens, prompt_adapter_args
        )
        iou_token_out = hs[:, 0, :]
        ...
        mask = (hyper_in[:, :4] @ upscaled_embedding_sam.view(b, c, h * w)).view(
            b, -1, h, w
        )
